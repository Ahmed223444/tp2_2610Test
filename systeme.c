...
void restoreSystem() {
    if (!systemCrashed) {
        printf("[SYSTEME] Aucun besoin de restauration.\n");
        return;
    }
...
int main() {
    ...
    while (1) {
        ...
        switch (command) {
            case 'p': raise(SIGUSR1); break;
            case 'r': restoreSystem(); break;
            case 'q': shutdownSystem(); return 0;
            default: printf("[SYSTEME] Commande inconnue.\n");
        }
